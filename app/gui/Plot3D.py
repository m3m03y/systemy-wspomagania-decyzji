...
class PlotDialog3D(QDialog):
    ...
    def accept(self):
        ...
        if (chosen_column_x != None and chosen_column_y != None and chosen_column_z != None) and (chosen_column_x != '' and chosen_column_y != '' and chosen_column_z != ''):
            print(
                f'Plot 3D Dialog:: Selected column x: {chosen_column_x}  y: {chosen_column_y} z: {chosen_column_z}')
            self.columns_chosen.emit(chosen_column_x, chosen_column_y, chosen_column_z)
...
class Display3DPlot(QDialog):
    def __init__(self, x_range, y_range, z_range, x_name, y_name, z_name):
        super().__init__()
        self.x = x_range
        self.y = y_range
        self.z = z_range
